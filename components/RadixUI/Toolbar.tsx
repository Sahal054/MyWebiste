...
export const Toolbar = ({ elements, className, 'aria-label': ariaLabel }: ToolbarProps) => {
    return (
        <RadixToolbar.Root
            ...
            className={`flex w-full min-w-max rounded bg-primary p-1 border border-primary ${className || ''}`}
                ...
                if (element.type === 'button') {
                    return (
                        <OSButton
                            ...
                            hover="background"
                            icon={element.icon}
                            className={`${element.className || ''} !px-[5px] ${element.active ? '!bg-accent text-primary' : ''}`}
                            disabled={element.disabled}
                        >

...
const checkTrashIntersection = (info: any) => {
    ...
    return (
        ...
        >
            <div className={`flex items-center justify-center overflow-hidden ${isPlainImageIcon ? 'w-12 h-12 bg-transparent border-0 shadow-none rounded-none' : 'w-14 h-14 bg-white border-2 border-primary shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-xl group-hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-shadow'}`}>
                {app.iconUrl ? (
                    <img src={app.iconUrl} alt={app.label} className={isPlainImageIcon ? 'w-12 h-12 object-contain' : 'w-10 h-10 object-contain'} draggable={false} />
                ) : (
                    <AppIcon name={app.label} className="w-8 h-8 text-primary" />
                )}
            </div>
            
            <span className={`text-xs font-bold text-primary truncate max-w-full text-center select-none border border-transparent ${isPlainImageIcon ? 'bg-transparent px-0 py-0 rounded-none' : 'bg-background/80 px-2 py-0.5 rounded backdrop-blur-sm group-hover:border-primary/20'}`}>
                {app.label}
            </span>
        </motion.div>
    )

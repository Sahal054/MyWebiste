...
export default function PdfWindow() {
    const {
        isPdfWindowOpen, setPdfWindowOpen,
        isPdfWindowMinimized, setPdfWindowMinimized,
        activePdfDocId, setActivePdfDocId,
        savedDocs, certificationDocs,
    } = useApp()
    const dragControls = useDragControls()
    const [isMaximized, setIsMaximized] = useState(false)
    const W = 760, H = 560

    const doc = [...savedDocs, ...certificationDocs].find(d => d.id === activePdfDocId)

    if (!doc || !doc.filename.toLowerCase().endsWith('.pdf')) return null
    ...
    return (
        <AnimatePresence>
            {isPdfWindowOpen && !isPdfWindowMinimized && (
                <motion.div
                    drag={!isMaximized}
                ...
                >
                    ...
                    >
                        <div className="flex items-center gap-1.5 group/lights">
                            ...
                            <button
                                onPointerDown={e => e.stopPropagation()}
                                onClick={() => setPdfWindowMinimized(true)}
